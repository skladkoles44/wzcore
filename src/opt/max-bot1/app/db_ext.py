    ...
    try:
        ...
        if row is None:
            c.execute(
                """INSERT INTO users_profile(
                      ...
                      display_name, username, login, first_name, last_name,
                      is_bot, last_chat_id, last_text, sender_json, is_admin
                   ) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?)""",
                (
                    user_id,
                    now,
                    now,
                    (sender.get("display_name") or sender.get("first_name") or sender.get("name") or ""),
                    ...
                    (chat_id or 0),
                    (text or ""),
                    (json.dumps(sender, ensure_ascii=False)[:8000]),
                    0,  # is_admin is managed in DB (users_profile.is_admin)
                ),
            )
            c.commit()
            return True
        ...
        c.execute(
            ...
            (
                ...
                (text or ""),
                (json.dumps(sender, ensure_ascii=False)[:8000]),
                None,  # do not overwrite is_admin (keep DB value)
                user_id,
            ),
        )
        c.commit()
